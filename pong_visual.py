...
class Agent:
    ...
    def _reset(self):
        # gym.reset() may return either obs or (obs, info)
    ...
    def step_env(self, net, epsilon, render=False):
        ...
        # gym.step() may return 4 or 5 values
        ...
        self.buffer.append(
            Transition(self.state, action, reward, done, new_state)
        )
        self.state = new_state

        if done:
            finished_reward = self.total_reward
            self._reset()
